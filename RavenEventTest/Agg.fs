...
/// An interface to NEventStore that emits and consumes TEvents
  ...
  member this.load id = 
    ...
    stream.StreamRevision, seq {
      for eventmsg in stream.CommittedEvents do
        ...
        | :? 'TEvent as e -> yield e
        | _          as e -> failwith <| sprintf "Didn't get expected event type %s.  Instead got %s. Value:\n%A"
                                                 (typeof<'TEvent>.ToString()) (e.GetType().ToString()) e 
      }
...
type NEventAggregate<'TState, 'TCommand, 'TEvent>(agg:Aggregate<'TState,'TCommand,'TEvent>, store) =
  ...
  /// Return a revision and TEvent object hydrated from the stream, or an empty (zero) TEvent object for nonexistant streamids
  member this.Load streamid =
    let revision, events = loader.load streamid
    let state = events |> Seq.fold agg.apply agg.zero
    (revision, state)

  /// Return the commitid, resulting TEvent, and new TState after attempting to execute a TCommand against the aggregate.
